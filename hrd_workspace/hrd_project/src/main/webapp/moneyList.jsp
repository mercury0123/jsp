<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@page import="java.sql.PreparedStatement"%>
<%@page import="java.sql.Connection"%>
<%@page import="java.sql.ResultSet"%>
<%@page import="java.sql.DriverManager"%>

<%
    //DB연결정보
	Connection conn = null;
	PreparedStatement pst = null;
	ResultSet rs = null;	
	
	String sql ="select m1.custno, m1.custname, m1.grade, sum(m2.price)as hap from member_tbl_02 m1 inner join money_tbl_02 m2 	on m1.custno = m2.custno group by m1.custno, m1.custname, m1.grade order by hap desc";
	try {
		Class.forName("oracle.jdbc.OracleDriver");
		conn = DriverManager.getConnection("jdbc:oracle:thin:@//localhost:1521/xe","system","ezen");		
		pst = conn.prepareStatement(sql);
		rs = pst.executeQuery(); //select 는 executeQuery()
		
		
		%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Money List</title>
<link href="style.css" rel="stylesheet">
</head>
<style>
	header {
	 	position: fixed;
	}
	nav{
		position: fixed;
	}
	section {
		display : block;
		position: fixed;
		height: 100%;
	}
	footer {
		position: fixed;
	}
</style>
<body>
<jsp:include page="header.jsp"></jsp:include>
<section>
<h2>회원매출조회</h2>
<table>
<tr>
	<th>회원번호</th>
	<th>회원성명</th>
	<th>고객등급</th>
	<th>매출</th>
</tr>
	<%
	while(rs.next()){
		//고객 등급에 대한 분류
		String grade = "";	//"" 대신 직원 값을 default 값으로 넣어도 된다
		if(rs.getString(3).equals("A")) grade="VIP";	//3번째 값 고객등급이 A와 같다면
		if(rs.getString(3).equals("B")) grade="일반";		//3번째 값 고객등급이 B와 같다면
		if(rs.getString(3).equals("C")) grade="직원";		//3번째 값 고객등급이 C와 같다면			


	%>
<tr>
	<td><%= rs.getInt(1) %></td>
	<td><%= rs.getString(2) %></td>
	<td><%= grade%></td>
	<td><%= rs.getInt(4)%>원</td>
</tr>
	<%
	}	//while 여기서 닫아줌
	%>
</table>
</section>
<jsp:include page="footer.jsp"></jsp:include>
<%		
	} catch (Exception e){	
		e.printStackTrace();	
	} finally {
		if(rs!=null) rs.close();
		if(pst!=null) pst.close();
		if(conn!=null) conn.close();
	}
	 %>
</body>
</html>